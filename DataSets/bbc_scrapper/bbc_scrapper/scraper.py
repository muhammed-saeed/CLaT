...
def get_urls(
    ...
    print("article_count_span", article_count_span)
    if article_count_span:
        ...
        for count in range(1, total_article_count):
            try:
                page_soup = get_page_soup(category_url[:-1] + f"{count+1}")
...
def get_valid_urls(category_page:BeautifulSoup) -> List[str]:
    ...
    # all_urls = category_page.findAll("a",attrs={"class": CONFIG["ARTICLE_TITLE_CLASS"]})
    valid_article_urls = []
    for url in all_urls:
        href = url.get("href")
        # valid_article_urls.append(href)
...
def get_article_data(article_url:str) -> Tuple[Optional[str], Optional[str], str]:
    ...
    if story_div:
        ...
        story_text = clean_str(story_text)
